...
def main() -> int:
    parser = argparse.ArgumentParser(description="View outputs from src/main_spectral.cpp")
    parser.add_argument("--run", action="store_true", help="Run igneous-spectral before rendering")
    parser.add_argument("--executable", default="build/igneous-spectral")
    parser.add_argument("--input-obj", default="assets/bunny.obj")
    parser.add_argument("--output-dir", default="output_spectral")
    parser.add_argument("--plots-dir", default="visualizations/results/main_spectral")
    ...
    files = sorted(output_dir.glob("eigen_*.ply"))
    if not files:
        raise SystemExit("No eigen_*.ply outputs found. Run with --run or verify --output-dir.")
    ...
    write_index(index_path, "main_spectral outputs", rows)
